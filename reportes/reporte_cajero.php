...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(52,10,'REPORTE DE CAJA',0,0,'C');
		
		$this->SetDrawColor(31,78,121);
		$this->SetLineWidth(0.5);
		$this->Line(81.00,23.00, 131.00, 23.00);

		$this->Ln(40);
		//$this->Cell(10);
		$this->SetFont('Arial','B',8);
		$this->Cell(6,7, '#', 1, 0, 'C',0);
		$this->Cell(15,7, 'NIT/CI', 1, 0, 'C',0);
		$this->Cell(65,7, ' RAZON SOCIAL', 1, 0, 'L',0);
		$this->Cell(50,7, ' PLAN', 1, 0, 'L',0);
		$this->Cell(30,7, 'FEC COBRANZA', 1, 0, 'C',0);
		$this->Cell(15,7, 'ESTADO', 1, 0, 'C',0);
		$this->Cell(13,7, 'MONTO', 1, 1, 'R',0);
...
/* 
	...
	$cajero = 'Juan Perez';
 */	
	$rows = getData($conexion, $fec_ini, $fec_fin, $codigo_agencia,$id_usuario);
	...
	//$pdf->Cell(5);
	$pdf->Cell(5);
	$pdf->Cell(35,7,'Cajero: ' . $usuario , 0, 0, 'L',0);
	$pdf->Cell(105);
	...
	while($row = mysqli_fetch_assoc($rows)){
		...
		$pdf->Cell(15,7, $row['cedula'], 1, 0, 'C',0);
		$nombre = utf8_decode($row['nombre']);
		$pdf->Cell(65,7, $nombre, 1, 0, 'L',0);
		$plan = utf8_decode($row['plan']);
		$pdf->Cell(50,7, $plan, 1, 0, 'L',0);
		$pdf->Cell(30,7, $row['fechaCobranzas'], 1, 0, 'C',0);
		$pdf->Cell(15,7, $row['estado'], 1, 0, 'L',0);
		$precio = number_format($row['precio'],2,",",".");
		$pdf->Cell(13,7, $precio, 1, 1, 'R',0);
	...
	$pdf->Cell(34,7, 'TOTAL Bs.: '.$total, 1, 1, 'R',0);
