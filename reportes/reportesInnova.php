...
if(isset($_POST['exportar'])){
	...
	$rows = getData($con,$f_ini, $f_fin, $codigo_canal,$codigo_agencia);
	...
	while ($student = mysqli_fetch_assoc($rows)){
		...
		$spreadsheet->getActiveSheet()
			...
			->setCellValue('P'.$row , $student['estado']);
...
function getData($db, $f_ini, $f_fin, $codigo_canal, $codigo_agencia){
	...
	if($codigo_agencia == 'TODO'){
		...
		$sql = "SELECT t.id, a.codigo_agencia, a.nombre_agencia as agencia, k.nombre_ciudad as ciudad,
					CASE 
						WHEN t.codigo_plan = 'PC01' THEN 'PLAN BIENESTAR Y PROTECCION'
						WHEN t.codigo_plan = 'PC02' THEN 'PLAN BIENESTAR Y PROTECCION MULTIANUAL'
						WHEN t.codigo_plan = 'PC03' THEN 'PLAN ELLA MULTIANUAL'
            			WHEN t.codigo_plan = 'PC04' THEN 'PLAN EL MULTIANUAL'
					END as plan, t.precio,t.fecha_creacion as fechaInicio, t.fecha_cobranzas as fechaCobranzas, 
					t.fecha_facturacion as fechaFacturacion,
					CONCAT(nombres, ' ',ap_paterno, ' ',ap_materno) nombre, c.tipo_documento, c.num_documento as cedula, 
					c.genero, c.fecha_nacimiento,c.telefono, u.nombre as usuario, 
					CASE 
