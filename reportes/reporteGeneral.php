 ...
 */
	...
	while ($student = mysqli_fetch_assoc($rows)){
		...
		$codigo_agencia = $student['codigo_agencia'];
		$agencia = $student['agencia'];
		$ciudad = $student['ciudad'];
		$plan = $student['plan'];
		$fechaInicio = $student['fechaInicio'];
		$fechaCobranzas = $student['fechaCobranzas'];
		...
		$cedula_asesor = $student['cedula_asesor'];	 // CI Asesor MALY 29/02/2024


		$spreadsheet->getActiveSheet()
			->setCellValue('A'.$row , $i)
			->setCellValue('B'.$row , $codigo_agencia)
			->setCellValue('C'.$row , $agencia)
			->setCellValue('D'.$row , $ciudad)
			->setCellValue('E'.$row , $plan)
			->setCellValue('F'.$row , $precio)
			->setCellValue('G'.$row , $fechaInicio)
			->setCellValue('H'.$row , $fechaCobranzas)
			->setCellValue('I'.$row , $nombre)
			->setCellValue('J'.$row , $tipo_documento)
			->setCellValue('K'.$row , $cedula)
			->setCellValue('L'.$row , $genero)
			->setCellValue('M'.$row , $fecha_nacimiento)
			->setCellValue('N'.$row , $telefono)
			->setCellValue('O'.$row , $cedula_asesor) // CI Asesor MALY 29/02/2024				
			->setCellValue('P'.$row , $usuario)
			->setCellValue('Q'.$row , $estado);
